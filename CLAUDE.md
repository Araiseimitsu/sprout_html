...
UI層とAPI層、FrontendとBackend、SvelteKitとFastAPIで責務を二重化しない
特に API呼び出し、入力検証、状態管理、データ変換、認証、ルーティング については、正本となる実装箇所を1つに固定する
SvelteKitを利用する場合でも、FastAPIをバックエンドとして採用している構成では、SvelteKitのサーバー機能を安易に追加してAPIや業務ロジックを重複実装しない
...
FastAPI が存在する構成で、同等のAPIや業務ロジックを SvelteKit 側に重ねて実装すること
