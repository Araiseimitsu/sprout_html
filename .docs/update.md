...
- Frontend: AIモデル候補を `.env` で変更した場合も、候補一覧の先頭を標準、2番目を高性能として選択できるように修正。`gemini-3.5-flash` 固定判定を廃止し、候補が2つ以上ある場合に高性能モードを有効化。
- Frontend: `GEMINI_TEXT_MODEL` に高性能モデルを指定しても、標準モデルと高性能モデルが同一にならないよう、モード選択時は候補一覧の先頭/2番目を使用するよう修正。
...
- Backend: Gemini連携サービス(ai_service)を追加。HTMLゼロ生成・全体編集・部分編集・画像生成(gemini-3.1-flash-lite-image)を提供。APIキー/モデル呼び出しはバックエンドに集約。
